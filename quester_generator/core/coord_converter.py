...
ZONE_DIMENSIONS = {
    ...
    188:  {'map': 1, 'left': 10666.67, 'right': 8533.33, 'top': 12266.67, 'bottom': 10133.33},  # Shadowglen (Subzone)
...
def is_coords_in_bounds(zone_id: int, x: float, y: float) -> bool:
    ...
    # Добавляем буфер (500), чтобы не терять точки на границах
    min_x = min(dims['bottom'], dims['top']) - 500
    max_x = max(dims['bottom'], dims['top']) + 500
    valid_x = min_x <= x <= max_x

    # Проверка Y (между Right и Left)
    min_y = min(dims['right'], dims['left']) - 500
    max_y = max(dims['right'], dims['left']) + 500
